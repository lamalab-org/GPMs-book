---
title: "The Shape and Structure of Chemical Data"
---
...
successes and failures of [ml]{acronym-label="ml"
acronym-form="singular+short"} models, it is instructive to explore how
...
concepts in [sm]{acronym-label="sm" acronym-form="singular+short"}.
...
<img src="media/figures/rescaled_figures/chemrev_figure1.png" width="100%" />
...
[gpms]{acronym-label="gpm" acronym-form="plural+short"} with soft
inductive biases---which guide the model toward preferred solutions
without enforcing strict constraints on the solution
space\[@wilson2025deep\]---are more suitable. [llms]{acronym-label="llm"
acronym-form="plural+short"} fall into this category.
...
lab and the datasets on which [ml]{acronym-label="ml"
acronym-form="singular+short"} models for chemistry are trained. Much of
...
explicit (reducible complexity). Such datasets, for
...
Considering the largest chemistry text dataset, ,\[@mirza2025chempile0\]
we can look at how the largest subsets fare in comparison to the
smallest ones (see
...
+--------------------------------------------------+-------------------+
| [ord]{acronym-label="ord"                        | 5,347,195,320     |
| acronym-form="singular+short"}\[@Kearnes_2021\]  |                   |
| reaction prediction                              |                   |
+--------------------------------------------------+-------------------+
| molecular features                               | 5,000,435,822     |
+--------------------------------------------------+-------------------+
...
+--------------------------------------------------+-------------------+
| [ord]{acronym-label="ord"                        | 8,372             |
| acronym-form="singular+short"}\[@Kearnes_2021\]  |                   |
| recipe yield prediction                          |                   |
+--------------------------------------------------+-------------------+

: **Token counts for the three largest and smallest datasets in the
\[@mirza2025chempile0\] collection.** Dominating datasets contribute a
large portion of the total token count (a token represents the smallest
unit of text that a [ml]{acronym-label="ml"
acronym-form="singular+short"} model can process), with the small
...
<img src="media/figures/final_figures/2_DATA_cumulative_histogram.png" width="100%" />
<figcaption><strong>Cumulative token count based on the tabular datasets
[@mirza2025chempile0]</strong>. We compare the approximate token count
for three datasets: training dataset,[@grattafiori2024llama] openly
available chemistry papers in the dataset, and the dataset. As can be
seen, by aggregating the collection of tabular datasets converted to
text format in the subset, we can achieve the same order of magnitude as
the collection of open chemistry papers. However, without smaller
datasets, we cannot capture the breadth and complexity of chemistry
data, which is essential for training <span data-acronym-label="gpm"
data-acronym-form="singular+short">gpm</span>. The tokenization method
for both and is provided in the respective papers.</figcaption>
</figure>
...
tail with conventional [ml]{acronym-label="ml"
acronym-form="singular+short"} techniques. However, the promise of
[gpms]{acronym-label="gpm" acronym-form="plural+short"} such as
[llms]{acronym-label="llm" acronym-form="plural+short"} is that they can
...
[gpms]{acronym-label="gpm" acronym-form="plural+short"}, data can---but
should not---be directly ingested in a raw format and requires some form
of pre-processing. Training [gpms]{acronym-label="gpm"
acronym-form="plural+short"} typically requires a large and diverse
...
resources such as \[@commoncrawl\]---is filtered using custom-built
procedures (e.g., using regular expressions or classification models).
This approach is gaining traction in the development of foundation
models such as [llms]{acronym-label="llm"
acronym-form="plural+short"}.\[@penedo2023refinedweb;
...
the performance of [gpms]{acronym-label="gpm"
acronym-form="plural+short"}.\[@maini2024rephrasing;
...
very popular in [ml]{acronym-label="ml" acronym-form="singular+short"}
for chemistry.
...
<img src="media/figures/rescaled_figures/chemrev_figure3.png" width="100%" />
...
data-acronym-form="plural+short">gpms</span>. The “bottom-up” approach
...
[gpms]{acronym-label="gpm" acronym-form="plural+short"} are currently
...
[gpms]{acronym-label="gpm" acronym-form="plural+short"}. While initially
...
example, @shao2024deepseekmath0 filtered for mathematical text using a
combination of regular expressions and a custom, iteratively trained
classification model\[@bojanowski2017enriching\]. An alternative
approach was pursued by @thrush2024improving who introduced a
training-free framework. In this method, the pre-training text was
...
text)---as scored by $90$ publicly-available [llms]{acronym-label="llm"
acronym-form="plural+short"}---with downstream benchmark accuracy,
keeping only the high-correlation domains.

In the chemical domain, \[@mirza2025chempile0\] is the only open-source,
pre-training scale dataset, that underwent several filtering steps. For
example, a large subset of the papers in come from the dataset. To
filter for chemistry papers, a custom classification model was trained
from scratch using topic-labeled data from the \[@li2023camel\] dataset.
To evaluate the accuracy of the model ($\text{F1-score}=0.77$),
...
[ml]{acronym-label="ml" acronym-form="singular+short"} model.
...
\[@brinkhaus2022randepict\] have been used to create enriched datasets
of chemical representations. These tools generate human-like drawings of
...
manuscripts) and further augment them using conventional
...
where multiple [smiles]{acronym-label="smiles"
acronym-form="singular+short"} strings were mapped to a single property.
...
representations (e.g., [iupac]{acronym-label="iupac"
acronym-form="singular+short"} names or
[selfies]{acronym-label="selfies" acronym-form="singular+short"}), but
historically, [smiles]{acronym-label="smiles"
acronym-form="singular+short"} has been used more often. As a result,
...
[lm]{acronym-label="lm" acronym-form="singular+short"}. Another
...
auto-formalization\[@NEURIPS2022_d0c6bc64\], a [llm]{acronym-label="llm"
acronym-form="singular+short"}-powered approach that can turn
natural-language mathematical proofs into computer-verifiable
mathematical languages such as \[@de2015lean\] or
\[@wenzel2008isabelle\]. Such datasets have been utilized to advance
mathematical capabilities in [lms]{acronym-label="lm"
acronym-form="plural+short"}.\[@xin2024deepseek; @trinh2024solving\]
