...
\acr{rag} (see
@sec-model_adaptation).\[@lewis2020retrieval\]
...
tools (@sec-agents). Such systems are typically described as being
...
@sec-embeddings, solve this through learned, dense vector
representations.
...
[@sec-syn-data]), it can also confuse models because the same
...
[@sec-prediction]) to molecular generation (see
[@sec-mol_generation]). The choice of embedding strategy often
...
@fig-training_workflow). \[@howard2018universal\] The first
...
@sec-pretraining for more details), it learns to create an
internal representation---an embedding (see
@sec-embeddings). The training process guides the model to
...
@fig-training_workflow, the fundamental shape of the
...
@fig-training_workflow, this biases the output towards a
...
tokens, see
[@fig-scale_of_data]). The model learns to map input to
...
@fig-training_workflow, the pre-training process creates a
...
@fig-types_ssl illustrates the two main families: generative
...
@fig-types_ssl include predicting masked portions of a
...
input, which establishes the conditional context.

![](media/eq_images/nexttoken.png)
...
loss functions like `InfoNCE`. \[@oord2018representation\] The loss function is formulated as a
...
generative masked language model learns the conditional probability, aiming to assign a high probability to the
...
viewed as a log-loss for a $(K+1)$-way classification task. Here, the model learns to identify the
...
@sec-multimodal_chem), alignments between different
...
@fig-training_workflow). This specialization process
...
shape (@fig-training_workflow), but the model learns a new way to
navigate it, guided by the reward.

![](media/eq_images/rl_objective.png)
...
- **Policy (π or PLLM)** (Red): The LLM itself, whose parameters define the probability
distribution over possible following tokens given the current state. This is what
gets optimized during training.

- **Reward (R)** (Peach): A numerical score assigned to complete generated sequences,
measuring how well the output achieves the desired goal. Used only
during training to guide parameter updates.

- **Expectation (E)** (Peach): The average cumulative reward, calculated over many
possible sequences that could be generated by the current policy.

- **Training Objective**: Adjust the model’s parameters to maximize the expected
cumulative reward (E[R]) over complete sequences, enabling the generation of
...
policy is the \acr{llm} itself. Consider teaching a model to
...
$\pi(a|s) = P_{\text{LLM}}(\text{next synthetic step}|\text{current plan})$. The model leverages its chemical
...
@fig-architectures are examples of foundational backbones
...
- **Input**: Similar to \acr{lstm}s, data is tokenized and often enhanced
  ...
  @fig-architectures, the tokenized sequence is added with
...
@sec-common_representations). Molecules are represented as
...
@sec-contrastive_learning), is to align these separate
spaces.
...
@sec-model_adaptation) is trained to project the embeddings
...
[@sec-mol_generation] for more details on this application).
...
[@{sec-contrastive_learning}]"}. `MoleculeSTM`
...
@sec-model_adaptation) to project molecular information into
...
@sec-quantization) for even greater efficiency.
...
[]{#tbl-model_adaptation label="tbl-model_adaptation"}
...
@sec-fine_tuning_coloring), unlike techniques like
\acr{icl} or prompt
...
[@sec-planning]), and deciding which tools to use to
