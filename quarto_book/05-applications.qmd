...
entire scientific workflow. The stages outlined in
[1](#fig:applications){reference-type="ref+Label"
reference="fig:applications"}---from hypothesis generation to
...
deceiving. Finally, the problem's open-ended nature @sec-data-section makes systematic benchmarking ill-posed.
...
search algorithms such as \acr{bfs}, \acr{mcts}\[@segler2017towards\] to explore the
...
@fig-planning_example shows how a \acr{llm} applies this deliberate, system-2-style
...
decompose problem**. \acr{llm} decomposes a chemical
...
Example [\[example:chemcrow_organa\]](#example:chemcrow_organa){reference-type="ref"
reference="example:chemcrow_organa"} presents examples illustrating how
...
example, \acr{chidl}\[@mehr2020universal; @xdl2023spec\]
...
(example [\[example:chemcrow_organa\]](#example:chemcrow_organa){reference-type="ref"
reference="example:chemcrow_organa"}), and collaborated with chemists to
...
substeps that it coordinated in parallel (see
example [\[example:chemcrow_organa\]](#example:chemcrow_organa){reference-type="ref"
reference="example:chemcrow_organa"}). If an unexpected observation
...
- **Universal Protocol Standards.** No widely adopted formalization
  standard exists. While languages like \acr{chidl} show promise, achieving
