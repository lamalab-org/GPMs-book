...
@fig-applications, and detailed in the subsequent sections.

While significant applications emerge in \acr{ml} and programming, scientific
...
and `\acr{ai}``-Scientist`
\[@yamada2025ai\], aim to automate the entire \acr{ml} research pipeline, typically employing
multi-agent architectures (described in detail in
[@sec-multi-agent]) where specialized agents manage distinct
...
\[@novikov2025alphaevolve\], which is an \acr{llm} operating within a
\acr{ga} environment,
...
question: *What should the role of \acr{ai} in science be?* While these systems can
...
\[@livne2024nach0\]. Pre-trained using \acr{ssl} on both natural language and chemical
...
A second category of \acr{gpm}s emphasizes deep domain knowledge through
specialized pre-training. `LLaMat` employed \acr{peft} specifically on crystal structure data in
\acr{cif} format,
...
Unlike previous models, `ether0` uses \acr{rl} (see
[@sec-rl]) to
...
@fig-knowledge_gathering. We split the discussion in this
section in two: structured data extraction and question answering.
Example queries for both sections are in
@fig-knowledge_gathering B.

::: {#fig-knowledge_gathering}

![](media/figures/rescaled_figures/chemrev_figure12.png){width=100%}

**A. a representation of a typical agent for scientific queries.** The \acr{llm} is the central piece of the system, surrounded by typical tools that improve its question-answering capabilities, together forming an agentic system. The tools represented in this figure are semantic search, citation traversal, evidence gathering, and question answering. Semantic search finds relevant documents. Evidence gathering ranks and filters chunks of text using \acr{llm}s. The citation traversal tool provides model access to citation graphs, enabling accurate referencing of each chunk and facilitating the discovery of additional sources. Finally, the question-answering tool (an <span data-acronym-label="llm" data-acronym-form="singular+short">llm</span>) collects all the information found by other tools and generates a final response to a user's query. This part the figure is inspired by the `PaperQA2` agent.[@skarlinski2024language] **B.** two examples of applications discussed in this section.
:::
...
[@sec-embeddings]).\[@bojanowski2017enriching\] They represent
...
avenues for the use of \acr{llm}s in the chemical sciences
\[@schilling2025text\]. Currently, \acr{llm}s are used in various forms for this
application.
...
\acr{uspto}, and
converted it to a \acr{json} format compatible with the schema of
\acr{ord}\[@Kearnes_2021\], achieving an overall
...
@fig-knowledge_gathering. The evidence-gathering tool
...
discussed in
[@sec-data-section], risk intractability, as an unbounded
...
frameworks (see
[@sec-multi-agent]). For instance, @Ghafarollahi2024
...
[@sec-prompting]) for hypothesis generation and compared it
...
**Overview of \acr{llm}-based hypothesis generation**. Current methods are based on <span data-acronym-label="llm" data-acronym-form="singular+short">llm</span>-sampling methods in which an \acr{llm} proposes new hypotheses. The generated hypotheses are evaluated in terms of novelty and impact either by another \acr{llm} or by a human. Then, through experimentation, the hypotheses are transformed into results which showcase that current \acr{llm}s cannot produce groundbreaking ideas, limited to their training corpus, resulting in the best cases, in incremental work. This is shown metaphorically with the puzzle. The "pieces of chemical knowledge" based on the hypothesis produced by \acr{llm}s are already present in the "chemistry puzzle", not unveiling new parts of it.
:::
...
success in \acr{ai}-generated ideas accepted at workshops in
\acr{ml} conferences via
...
the strongest domains for \acr{llm}s. Second, workshop acceptances are less
...
@fig-hypothesis-generation. Thus, while accidental
...
[@sec-safety]) \[@danish_gov2024hypothesis\].
...
1960s, systems like \acr{lhasa} \[@corey1972computer\] began automating
...
various forms of search algorithms such as \acr{bfs}, \acr{dfs}, \acr{mcts} \[@segler2017towards\]. Those, however,
...
slower, more analytical process, in which \acr{llm}s solve complex tasks---such as logical
...
\acr{llm}s during
inference. Methods such as \acr{cot} and least-to-most prompting guide models
...
proposed to enhance planning capabilities in \acr{llm}s. Repeated sampling allows the model to
...
Beyond purely linguistic reasoning, \acr{llm}s have also been used to interpret
natural-language queries and to translate them into structured planning
steps, as demonstrated by systems like \acr{llm}+P\[@liu2023llm\] and
\acr{llm}-DP\[@dagan2023dynamic\], which integrated
\acr{llm}s with classical
planners to convert planning problems into \acr{pddl}. \acr{llm}s have also been applied to generate
...
In addition to external tools, \acr{llm}s can also engage in self-correction, a
...
evaluation, comparing \acr{llm}-generated protocols to expert-written
...
machine code (instructions) for the \acr{cpu}, executed to produce data, and finally
...
using a combination of fine-tuning, \acr{cot} and \acr{rag} (see
[@sec-model_adaptation]) can improve the performance of
...
Examples of \acr{gpm}-based autonomous agents (see
[@sec-agents]) capable of performing the entire computational
...
a human or a \acr{gpm}---to low-level instructions before
execution, while in interpreted automation, the
\acr{gpm} plays a central
...
![](media/figures/rescaled_figures/chemrev_figure15+16.png){width=100%}

**Programming languages vs. lab automation. A) programming paradigms**: In compiled languages, the entire source code is translated ahead of time to machine code by the compiler. This stand-alone code is then given to the \acr{os}, which is responsible for scheduling and distributing tasks to the hardware. In interpreted languages, the interpreter reads and translates each line of the source code to machine code and hands it to the \acr{os} for execution. **B) automation paradigms**: In the compiled approach, a <span data-acronym-label="gpm" data-acronym-form="singular+short">gpm</span> formalizes the protocol, a compiler, such as the chempiler[@steiner2019organic], translates the formalized protocol to hardware-specific low-level steps, which the controller then executes—a central hub tasked with scheduling and distributing commands to chemical hardware. In the interpreted approach, a \acr{gpm}, acting as the interpreter, first breaks down the protocol into specific steps, then sends them (via an \acr{api}) for execution one by one. The strength of interpreted systems is dynamic feedback: after the execution of each step, the \acr{gpm} receives a signal (e.g., data, errors), which can influence its behavior for the next steps.
:::

### Compiled Automation

In the case of "compiled automation", the experiment protocol needs to
be formalized in a high-level or \acr{dsl} that describes exactly what operations to
...
specialized languages (\acr{dsl}s) have also been developed to provide more
...
such languages is \acr{chidl}\[@xdl2023spec\], developed as part of the
Chemputer architecture \[@steiner2019organic; @mehr2020universal;
@hammer2021chemputation\]. \acr{chidl} uses a \acr{json}-like format, and the experimental
...
approach, the role of the \acr{gpm} is to translate natural-language
...
natural-language protocol into \acr{chidl} script, then automatically verifies its
...
\acr{gpm} agent (see
[@sec-agents]) can break it down into smaller steps and
...
`Coscientist` \[@boiko2023autonomous\] is an \acr{llm}-based chemistry assistant built around
`GPT-4` that can autonomously design and execute experiments. It can
take high-level goals and call tools to write code in real-time in order
to control an Opentrons OT-2 liquid-handling robot. The architecture
included a web-search module, a documentation module (to read instrument
manuals), a `Python` execution module (to run generated code in a
...
`GPT-4` would debug its own code. `Coscientist` successfully planned and
executed multistep syntheses with minimal human intervention. For
example, it efficiently optimized a palladium cross-coupling reaction
with minimal human input, outperforming a standard Bayesian optimizer
...
of an insect repellent, \acr{deet}, and three different organocatalysts and
...
a formal machine-readable format such as \acr{chidl}. Once validated, the formalized protocol
is compiled and executed. After the completion of execution, the
\acr{gpm} receives the
...
against mistakes made by the \acr{gpm} interpreter; any generated procedure must
...
given in @tbl-execution_comparison.

::: {#tbl-execution_comparison}
...
[]{#tbl-execution_comparison label="tbl-execution_comparison"}
...
remains difficult, as \acr{gpm}s can misinterpret ambiguous instructions,
...
while languages like \acr{chidl} show promise, inconsistencies in
...
[@sec-safety]. These challenges must be addressed in tandem to
...
promising candidate for \acr{gpm}s. They are designed to operate flexibly
...
**Static conventional data analysis workflow vs. dynamic \acr{gpm} generated workflow**. The chemical analysis can be done with a variety of possible instruments and techniques, resulting in a large number of possible output data formats. The \acr{gpm} can use these diverse, raw data and process it into easy-to-understand plots, analysis and reports. A hard-coded workflow, in contrast, is specifically made to analyze one specific data format and spectra and produces a fixed output format, e.g., the \acr{smiles} of the analyzed molecule.
:::

### Prompting

Initial evaluations demonstrated that \acr{gpm}s can support basic data analysis workflows.
\[@Fu2025large\] For example, in chemistry, this enabled the
classification of \acr{xps} signals \[@decurt2024large\] based on
...
direct interpretation by \acr{vlm}s a more natural starting point for automated
...
\acr{nmr}, and
\acr{xrd}q---directly
from images. They showed that while \acr{vlm}s can correctly extract isolated features
...
Beyond zero-shot prompting of \acr{gpm}s, one can develop agentic systems that
...
@fig-hypothesis-generation) and experimental analysis. The
data analysis agent `Finch` performs autonomous analysis of raw or
preprocessed experimental data, such as \acr{rna} sequencing and flow cytometry. Given a
user prompt (e.g., "\acr{rna} sequencing differential expression
...
significance testing, while for \acr{rna} sequencing, it encompasses differential
...
code, execute, and revise complete \acr{afm} analysis pipelines. The system handles
...
into scientific reports. Also, in this step, \acr{gpm}s can take a central role, which we discuss
in the following.
...
couple \acr{llm}s with
feature importance analysis tools, such as \acr{shap} or \acr{lime}. In this framework,
\acr{llm}s can additionally
interact with tools such as \acr{rag} over `arxiv` to provide evidence-based
explanations.

### Writing Assistance

When considering \acr{ml}-based assistance in scientific writing,
...
Under the latter mode, \acr{gpm} can be used to assist non-native English
...
Another application of \acr{llm} is to assist with completing checklists
before submitting a publication. For example, @goldberg2024usefulness
benchmark the use of \acr{llm}s in completing the author checklist for the
\acr{neurips} 2025.
...
\acr{gpm} has tremendous
potential. In @fig-writing_with_ml we showcase how the future of reporting
could look like if we were to integrate \acr{gpm} at each step of the process.

::: {#fig-writing_with_ml}

![](media/figures/rescaled_figures/chemrev_figure18.png){width=100%}

**Vision for \acr{gpm} in reporting, a visualization of the scientific writing process**. <span data-acronym-label="gpm" data-acronym-form="plural+short">gpms</span> can be used at every stage of the process. For creating the pre-print, we can utilize the multimodal capabilities of these models to write detailed captions for figures. For the peer-review process, we can harness the ability of \acr{gpm}s to summarize and prioritize information (e.g., design a time-efficient plan to address the peer review). When converting a document from a peer-reviewed pre-print, we often need to implement the publisher's requirements. In this case, we can make use of agentic systems that would assist with minor text fixes or document restructuring.
:::
