...
cycle (see
[@fig-applications]) hinges on their ability to streamline and
...
[@sec-common_representations] to learn in detail about the
...
[@sec-prompting]), fine-tuning (see
[@sec-fine-tuning]) on domain-specific data, and
\acr{rag} (see
[@sec-rag])
...
[@tbl-property_prediction_models] for properties predicted by
...
@srinivas2024crossmodal used zero-shot \acr{llm} prompting to generate
...
::: {}
![](media/eq_images/cot_prompting.png){width=100%}
:::
...
**Fine-tuned `GPT-3` for predicting solid-solution formation in high-entropy alloys** Performance comparison of different \acr{ml} approaches as a function of the number of training points. Results are shown for `Automatminer` (blue), `CrabNet` transformer (orange), fine-tuned `GPT-3` (red), with error bars showing standard error of the mean. The non-Google test set shows the fine-tuned `GPT-3` model tested on compounds without an exact Google search match (dark red). The dashed line shows performance using random forest. `GPT-3` achieves comparable accuracy to traditional approaches with significantly fewer training examples. Data adapted from @jablonka2024leveraging
:::
...
[@tbl-property_prediction_models]). They seminally
...
[@sec-example_architectures]). By pre-training on 91 million
...
[@sec-arch_agents] to read more about
\acr{react}), to
...
**Normalized error distributions for materials property prediction models across different architectures**. Each point represents the normalized error of a model on a specific property prediction task. Normalization was achieved with min/max values of each dataset to produce a range of errors between 0 and 1. The first column (blue) shows \acr{gnn} based models, the second column (red) displays \acr{llm} approaches, and the third column (orange) represents other baseline methods and <span data-acronym-label="sota" data-acronym-form="singular+short">sota</span> models including `CrabNet`. [@Wang_2021] Lower values indicate better predictive performance. Data adapted from @alampara2024mattext
:::
...
@fig-property_limitations. This observation suggests that
...
@fig-generation shows, critical bottlenecks persist in
...
[@sec-fine-tuning] for a deeper explanation of
\acr{lift} and
@sec-prediction_FT for a discussion of
\acr{lift} applied to
...
[@sec-arch-moes] to learn more about \acr{moe} architectures). `MOL-MOE` dynamically
...
[@sec-agents], have emerged as approaches for autonomous
...
@sec-retrosynthesis). However, these methods still face
...
[@sec-planning] to read more about this capability of
...
**Overview of the iterative optimization loop that mirrors the structure of the optimization section**. The blue boxes contain the different roles that the <span data-acronym-label="llm" data-acronym-form="plural+short">llms</span> play in the loop, and which are described in the main text. References in which the use of \acr{llm}s for that step are detailed inside the small boxes inside each of the components of the loop. The example shown is about obtaining molecules with high `logP`.
:::
...
@sec-retrosynthesis, those methods are used to accelerate
...
@sec-opt-llm-know-source). By fine-tuning the "worker"
...
@fig-optimization, this knowledge can be directed into
...
@sec-retrosynthesis). However, as the authors note, even
