...
::: {.callout-tip}
## Google search vs. LLM-assisted search

We most often use conventional search engines such as Google if we know what we search for (e.g. we want to look up opening hours of something). LLMs-assisted search tools (Perplexity, but also ChatGPT with web search) are best in cases where we do not even exactly know what we search for. 
:::
